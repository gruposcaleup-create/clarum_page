...
export default function TunnelShowcase() {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      requestAnimationFrame(() => {
        ...
        const w = canvasRef.current!.clientWidth || document.documentElement.clientWidth;
  ...
  return (
    <div 
      className="bg-transparent text-white min-h-screen overflow-hidden relative border-b border-white/10" 
      style={{ 
        marginTop: '-140px', 
        paddingTop: '140px',
        width: '100vw',
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen p-4 text-center">
        <div className={`${isMobile ? "mb-4 space-y-2" : "mb-8 space-y-3 md:space-y-6"}`}>
          <div className="inline-block">
            <h1 className={`${isMobile ? "text-4xl" : "text-6xl md:text-8xl"} font-bold tracking-tighter bg-gradient-to-r from-white via-gray-200 to-white bg-clip-text text-transparent animate-pulse`}>
              CLARUM
            </h1>
            <div className={`h-1 w-full bg-gradient-to-r from-transparent via-white to-transparent ${isMobile ? "mt-2" : "mt-4"} animate-pulse`} />
          </div>
          <p className={`${isMobile ? "text-sm px-4 leading-relaxed" : "text-lg md:text-xl px-0 leading-relaxed"} text-gray-300 max-w-2xl font-light mx-auto`}>
            Advancing the frontiers of intelligence and technology. 
            Experience an infinite journey through space and time.
          </p>
        </div>
      </div>
    </div>
